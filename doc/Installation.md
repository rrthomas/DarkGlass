...
You will also need the following Perl packages: CGI.pm and File::Slurp.
...
Then, configure the `web.pl` script as described in [Configuration](Configuration.html).
...
and any web server configuration files, are rendered verbatim. See [How output is generated from input](How output is generated from input.html) for more details.

See [Publishing a site](Publishing a site.html).

The site should now be ready to use. See [Testing](Testing.html) for how to test it in dynamic mode without configuring a web server.

Then, see [Customization](Customization.html) for details of the various ways in which DarkGlass can be customized.
