...
For each file, DarkGlass first decides whether to convert it to HTML. By default, it knows how to convert plain text, Markdown and LaTeX files, so given the files above, it will convert `Foo.html` and `README.html` to web pages. Other files are published as-is.
